...
#define ADDRESS_DEFAULT 0b1010010
...
bool VL53L0X::init(bool io_2v8)
{
  ...
  { 
    printf("Invalid VL53L0X identification model id.\r\n");  
    return false; 
  }
...
void VL53L0X::writeReg(uint8_t reg, uint8_t value)
{
  ...
  buf[0] = reg;
  buf[1] = value;
  if (0 != i2c.write(address, buf, 2))
  {
    printf("VL53L0X::writeReg - address: %02x, register: %02X, value: %02X FAILED\r\n", address, reg, value);
  }
}
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
  ...
  char value;
  i2c.read(address, &value, 1, false);
  return value;
}
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
  ...
  i2c.write(address, &data, 1, true);
  char value[2];
  i2c.read(address, &value[0], 2, false);
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
  ...
  i2c.write(address, &data, 1, true);
  char value[4];
  i2c.read(address, &value[0], 4, false);
...
{
   i2c.write(address, (const char*)src, count, false);
}
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
    const char data = reg;
    i2c.write(address, &data, 1, true);
    i2c.read(address, (char*)dst, 2, false);
}
